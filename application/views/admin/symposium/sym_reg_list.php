    ...
    <tbody>
      ...
      foreach($list as $value):?>
        <tr>
          ...
          <td><a href="reg_list?fk_symposium_no=<?=$value['no']?>&sym_title=<?=$value['title']?>" class="btn btn1">등록현황 보기</a></td>
        </tr>
      <?endforeach;?>
    </tbody>
    </table>
<!--//List-->
    ...
    <ul class="pagenation">
        <?php  if (count($list) > 0) {
         ...
         ?>
            <?php echo pagingView('/adm/symposium/sym_reg_list', $sPage, $_GET['page'], $ePage, $pageNum, $param)?>
        <?php } ?>
