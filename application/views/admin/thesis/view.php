...
<div class="board_view thesis_view">
    <ul class="bw_top">
        <li class="bw_subject"><?=$view[0]['title']?> </li>
        <li class="bw_date"><span class="date"><i class="i-calendar-check"></i>><?=date('Y-m-d', strtotime($view[0]['reg_date']))?></span><span class="count"><i class="i-eye"></i><?=$view[0]['view_count']?></span>
            <span class="state">
                <?
                switch($view[0]['status']) {
    ...
    <div class="comment_wrap">
        ...
        ?>
        <ul class="comment">
            <li class="cw_name"><i class="i-user"></i><?=$val['c_user_name']?></li>
    ...
    <div class="btn_wrap"> 
        <a href="/adm/thesis/list?<?=$param?>" class="btn btn1 ll">목록</a>
        <a href="/adm/thesis/write?<?=$param?>" class="btn btn1 rr">수정</a>
        <a href="javascript:del(<?=$view[0]['thesis_no']?>)" class="btn btn1 rr">삭제</a>
    </div>
    ...
    function del(no) {
        $.ajax({
          ...
          success: function (data) {
            ...
            if(data==1) {
                alert('삭제되었습니다');
                window.location.href="/adm/board/<?=$_GET['code']?>?<?=$param?>"
            } 
            else {
