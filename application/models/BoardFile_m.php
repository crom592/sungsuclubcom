...
class BoardFile_m extends CI_Model
{
    ...
    public function saveFile($finalFilesData=[], $owner_no, $oldFileArray=[])
    {
        ...
        foreach ($finalFilesData as $index => $value) {
            ...
            $this->db->delete('tbl_board_file', array('fk_board_no'=>$owner_no, 'no' => $index));

            $insertData = array(
                'file_name' => $value['client_name']
                ,'sys_file_name' => $value['file_name']
                ,'fk_board_no' => $owner_no
            ...
            $this->db->insert('tbl_board_file', $insertData);
