...
class Popup_m extends CI_Model
{
    ...
    public function totalCount($where='', $tableSet="tbl_popup")
    {
        if (!empty($where)) {
            $this->db->where($where);
        }
    ...
    public function list($where="", $sPage='', $ePage='', $orderBy=1, $tableSet="tbl_banner")
    {
        ...
        $this->db->from("tbl_popup bt");
        $this->db->join("tbl_popup_files tf", "tf.fk_popup_no=bt.no and tf.file_sno=1", "LEFT");
    ...
    public function view($where="", $tableSet="tbl_popup")
    {
        if (!empty($where)) {
            $this->db->where($where);
        }
    ...
    ## 글쓰기 저장
    public function store($insertData = [], $tableSet="tbl_popup")
    {
    ...
    ## 글쓰기 수정
    public function update($updateData = [], $tableSet="tbl_popup", $no)
    {
        $this->db->where("no=" . $no);
    ...
    {
        $this->db->trans_begin();
        $this->db->insert('tbl_popup', $insertData);
        ...
        if(empty($error_array['code']) ){
            ...
            if (count($upFile) > 0) {
                ...
                if (count($finalFilesData) > 0) {
                    ...
                    $error_check = $this->popupFile_m->saveFile($finalFilesData, $board_no, []);
                }
                if ($error_check > 0) {
    ...
    public function modify($updateData=[],$where="", $upFile=[], $oldFileArray=[], $oldFileCheckArray=[], $board_no)
    {
        ...
        $this->db->update('tbl_popup', $updateData);
        ...
        if(empty($error_array['code']) ){
            ...
            foreach ($oldFileCheckArray as $index => $value) {
                if (!empty($value)) {
                    ...
                    $this->db->delete('tbl_popup_files', array('no' => $value));
                }
            }
            if (count($upFile) > 0) {
                ...
                if (count($finalFilesData) > 0) {
                    $this->load->model('popupFile_m');
                    $error_check = $this->popupFile_m->saveFile($finalFilesData, $board_no, $oldFileArray);
                }
                if ($error_check > 0) {
                    $this->db->trans_rollback();
                    alert("시스템 오류! 관리자에게 문의(파일)");
    ...
    public function delete($no, $tableSet="tbl_popup")
    {
        if (!empty($where)) {
            $this->db->where($where);
        }
        ...
        if($rs) {
            $this->popupFile_m->deleteFile($no);
        }

        return $rs;
    }

    public function deleteArray($no_list=[], $tableSet="tbl_popup")
    {
