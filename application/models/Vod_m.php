...
class Vod_m extends CI_Model
{
    ...
    public function totalCount($where='', $tableSet="tbl_vod")
    {
        if (!empty($where)) {
            $this->db->where($where);
        }
    ...
    public function list($where="", $sPage='', $ePage='', $orderBy=1, $tableSet="tbl_vod")
    {
        if (!empty($where)) {
            $this->db->where($where);
        }
        ...
        $this->db->select("bt.*, tf.*, tu.user_name as user_name, tu.user_type");
        $this->db->from("tbl_vod bt");
        $this->db->join("tbl_vod_file tf", "tf.fk_board_no=bt.vod_no and tf.file_sno=1", "LEFT");
        $this->db->join("tbl_user tu", "tu.user_no=bt.reg_user_no", "LEFT");
        $this->db->group_by("bt.vod_no");
        $rs = $this->db->get();
        $rows = $rs->result_array();
        return $rows;
    }

    public function view($where="", $tableSet="tbl_vod")
    {
        if (!empty($where)) {
            $this->db->where($where);
        }
    ...
    ## 글쓰기 저장
    public function store($insertData = [], $tableSet="tbl_vod")
    {
    ...
    ## 글쓰기 수정
    public function update($updateData = [], $tableSet="tbl_vod", $no)
    {
        $this->db->where("no=" . $no);
    ...
    {
        $this->db->trans_begin();
        $this->db->insert('tbl_vod', $insertData);
        ...
        if(empty($error_array['code']) ){
            ...
            if (count($upFile) > 0) {
                ...
                if (count($finalFilesData) > 0) {
                    ...
                    $error_check = $this->vodFile_m->saveFile($finalFilesData, $board_no, []);
                }
                if ($error_check > 0) {
    ...
    public function modify($updateData=[],$where="", $upFile=[], $oldFileArray=[], $oldFileCheckArray=[], $board_no)
    {
        ...
        $this->db->update('tbl_vod', $updateData);
        ...
        if(empty($error_array['code']) ){
            ...
            foreach ($oldFileCheckArray as $index => $value) {
                if (!empty($value)) {
                    ...
                    $this->db->delete('tbl_vod_file', array('no' => $value));
                }
            }
            if (count($upFile) > 0) {
                ...
                if (count($finalFilesData) > 0) {
                    $this->load->model('vodFile_m');
                    $error_check = $this->vodFile_m->saveFile($finalFilesData, $board_no, $oldFileArray);
                }
                if ($error_check > 0) {
                    $this->db->trans_rollback();
                    alert("시스템 오류! 관리자에게 문의(파일)");
    ...
    public function delete($no, $tableSet="tbl_vod")
    {
        if (!empty($where)) {
            $this->db->where($where);
        }
        ...
        if($rs) {
            $this->db->delete('tbl_vod_comment', array('fk_board_no'=>$no));
            $this->load->model('vodFile_m');
            $this->vodFile_m->deleteFile($no);
        }
        return $rs;
    }

    public function deleteArray($no_list=[], $tableSet="tbl_vod")
    {
        
       
        foreach($no_list as $no) {

            $rs = $this->db->delete($tableSet, array('vod_no' => $no));
            if($rs) {
                $this->db->delete('tbl_vod_comment', array('fk_board_no'=>$no));
                $this->load->model('vodFile_m');
                $this->vodFile_m->deleteFile($no);
    ...
    public function updateCount($no)
    {
        ...
        $this->db->where('no', $no);
        $this->db->update('tbl_vod');
    }
    ## 글 삭제
    public function delComment($delData=[], $tableSet="tbl_vod_comment")
    {
        if (!empty($where)) {
            $this->db->where($where);
        }
