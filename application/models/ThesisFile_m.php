...
class ThesisFile_m extends CI_Model
{
    ...
    public function getFile($where="")
    {
        ...
        $rs = $this->db->get("tbl_thesis_file");
    ...
    public function saveFile($finalFilesData=[], $owner_no, $oldFileArray=[])
    {
        ...
        foreach ($finalFilesData as $index => $value) {
            ...
            $this->db->delete('tbl_thesis_file', array('fk_thesis_no'=>$owner_no, 'no' => $index));

            $insertData = array(
                'file_name' => $value['client_name']
                ,'sys_file_name' => $value['file_name']
                ,'fk_thesis_no' => $owner_no
            ...
            $this->db->insert('tbl_thesis_file', $insertData);
    ...
    ## 파일 삭제
    public function deleteFile($thesis_no)
    {
        $where = "fk_thesis_no='".$thesis_no."'";
        $rows = $this->getFile($where);
        foreach ($rows as $index => $value) {
            @unlink($_SERVER['DOCUMENT_ROOT'] . $value['file_path']);
        }
        $this->db->delete('tbl_thesis_file', array('fk_thesis_no'=>$thesis_no));
